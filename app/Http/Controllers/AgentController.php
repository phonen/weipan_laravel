...
class AgentController extends Controller
{
  ...
  public function home(Request $request)
  {
    ...
    $data = array(
      'today' => array(
        'users' => User::where('created_at', '>=', date('Y-m-d 00:00:00', time()))->where('id_introducer', $agent)->count(),
  ...
  public function users(Request $request)
  {
    ...
    if ($request->input('id_user', null)){
    	$datas->where('id_wechat', $request->input('id_user'));
    } elseif ($request->input('body_phone', null)) 
    {
    $datas->where('body_phone', $request->input('body_phone'));
    } elseif ($request->input('id_introducer', null)) 
    {
    $datas->where('id_introducer', $request->input('id_introducer'));
    }else{
    	 $datas->where('id_introducer', $agent);
  ...
  public function orders(Request $request)
  {
    ...
    $datas = Order::with('user')->orderBy('created_at', 'desc');
    	...
    	$datas->where('id', $request->input('id_order'));
    } elseif ($request->input('id_user', null)) 
    {
    $datas->where('id_user', $request->input('id_user'));
    } elseif ($request->input('id_object', null)) 
    {
    $datas->where('id_object', $request->input('id_object'));
    }else{
    	 $datas->where('id_object', '0');
  ...
  public function payRequests(Request $request)
  {
    ...
    if ($request->input('id_payRequest', null)){
    $datas->where('id', $request->input('id_payRequest'));
    } elseif ($request->input('id_user', null)) 
    {
    $datas->where('id_user', $request->input('id_user'));
    }else{
...
//    dd($datas);
  ...
  public function signIn(Request $request)
  {
    ...
    if ($request->isMethod('post')) {
      $agent = User::where('id_wechat', $request->input('id_wechat'))->where('password', md5($request->input('password')))->where('grade','1')->first();
      if ($agent) {
        $request->session()->put('agent', $agent->id_wechat);
        return redirect('/agent');
      }else
      {
      echo '<script type="text/javascript">';
      echo 'alert("不存在此代理商");';
      echo 'window.location.href="/agent/signIn";';
      echo '</script>';
